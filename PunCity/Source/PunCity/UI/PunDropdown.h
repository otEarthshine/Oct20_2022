...
public:
	...
	void Set(int32 objectId, std::vector<std::string> options, std::string selectedOption, std::function<void(int32, FString, IGameUIDataSource*, IGameNetworkInterface*)> onDropdownChanged)
	{
	...
	UFUNCTION() void OnDropdownChanged(FString sItem, ESelectInfo::Type seltype)
	{
		...
		_onDropdownChanged(punId, sItem, dataSource(), networkInterface());
	}

private:
	std::vector<std::string> _lastOptions;
	
	std::function<void(int32, FString, IGameUIDataSource*, IGameNetworkInterface*)> _onDropdownChanged;
};
