...
void TownManager::CollectRoundIncome()
{
	...
	// Change Influence
	int32 totalInfluenceIncome100_ExcludeManuallyChanged = totalInfluenceIncome100() - influenceIncomes100[static_cast<int>(InfluenceIncomeEnum::DiplomaticBuildings)]
																						...
																						- influenceIncomes100[static_cast<int>(InfluenceIncomeEnum::WorldTradeOffice)];
	
	globalResourceSys.ChangeInfluence100(totalInfluenceIncome100_ExcludeManuallyChanged);
...
void TownManager::RecalculateTax(bool showFloatup)
{
	...
	//! World Trade Office
	influenceIncomes100[static_cast<int>(InfluenceIncomeEnum::WorldTradeOffice)] = -100 * WorldTradeOffice::InfluenceUsagePerManipulation * _simulation->townBuildingFinishedCount(_townId, CardEnum::WorldTradeOffice);
}
	...
	if (command.callbackEnum == CallbackEnum::CancelTrainUnit)
	{
		for (int32 i = trainUnitsQueue.size(); i-- > 0;)
		{
			if (trainUnitsQueue[i].cardBirthTicks == command.variable2)
			{
				...
				if (isRealAction) {
					ResourcePair resourceCost = GetTrainUnitCost(trainUnitsQueue[i].cardEnum); // GetMilitaryInfo(trainUnitsQueue[i].cardEnum).resourceCost;
