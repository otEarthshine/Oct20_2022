...
void House::FinishConstruction()
{
	...
	//! Prevent building with disabled workforce during construction to disallow occupants to join
	...
	ExecuteOnLuxuryResourcesByTier([&](ResourceEnum resourceEnum, int32 tier)
	{
			...
			occupantCount() > 0)
		{
			...
			if (consumeLuxury) 
			{
				...
				actualConsumption = max(0, actualConsumption); // Ensure no negative if resourceCountWithPop is bugged
